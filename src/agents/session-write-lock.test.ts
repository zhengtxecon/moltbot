...
describe("acquireSessionWriteLock", () => {
  ...
  it("reclaims stale lock from same PID (container restart scenario)", async () => {
    ...
    // The lock should be reclaimed since it's not in HELD_LOCKS.
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbot-lock-pid-reuse-"));
    try {
      const sessionFile = path.join(root, "sessions.json");
      const lockPath = `${sessionFile}.lock`;

      // Write a stale lock file with current PID but old timestamp (> 5s grace period)
      await fs.writeFile(
        lockPath,
        JSON.stringify({
          pid: process.pid,
      ...
      // Should be able to acquire the lock despite same PID, because it's not in HELD_LOCKS
      const lock = await acquireSessionWriteLock({ sessionFile, timeoutMs: 2000 });

      // Verify the lock was reclaimed (new timestamp)
      const raw = await fs.readFile(lockPath, "utf8");
